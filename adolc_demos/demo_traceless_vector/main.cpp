...
int main() {
    ...
    // Set the desired number of independent variables (do it before declaring any adouble!)
    const int n = 10;

    // Prepare for traceless vector mode
    adtl::setNumDir(n);
    ...
    // Initialize active variables (seed with zeros to make sure)
    adouble f = 0.0;
    auto x = new adouble[n];
    for (int i = 0; i < n; ++i) {
        x[i].setValue(xp[i]);
        x[i].setADValue(i, seedZero);
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            if (i == j ) { x[i].setADValue(j, seedOne); }
            else { x[i].setADValue(j, seedZero); }
        }
